...
function generateSVG(symbol, values, metaData, periodLabel, themeName, isMini) {
  ...
  const priceTextColor = (dayColor === '#ffffff' || dayColor === '#a8ff78' || dayColor === '#00ff41' || dayColor === '#eab308' || dayColor === '#34d399' || dayColor === '#fb923c' || dayColor === '#f59e0b' || dayColor === '#06b6d4' || dayColor === '#fbbf24' || dayColor === '#2dd4bf') ? '#000000' : '#ffffff';
...
export default async function handler(req, res) {
  ...
  // Adding v3 to cache key to instantly bust the cache
  const cacheKey = `${symbol.toUpperCase()}_${numDays}d_${theme.toLowerCase()}_mini${isMini}_fake${isFake}_v3`;
