...
function generateSVG(symbol, values, metaData, periodLabel, themeName, isMini) {
  const width = isMini ? 400 : 850;
  const height = isMini ? 200 : 450;
  ...
  values.forEach((v, i) => {
    ...
    // Wick
    candlesHtml += `<line x1="${xCenter}" y1="${highY}" x2="${xCenter}" y2="${lowY}" stroke="${color}" stroke-width="${isMini ? 3 : 1.5}" />`;
    // Body
    candlesHtml += `<rect x="${xCenter - candleWidth/2}" y="${topBodyY}" width="${candleWidth}" height="${bodyHeight}" fill="${color}" rx="${isMini ? 2 : 1}" />`;
  });
...
export default async function handler(req, res) {
  ...
  const cacheKey = `${symbol.toUpperCase()}_${numDays}d_${theme.toLowerCase()}_mini${isMini}_fake${isFake}`;
  ...
  if (isFake) {
    values = generateFakeData(isMini ? 20 : numDays);
  ...
  if (isError || values.length === 0) {
    const errorSvg = `
      <svg width="${isMini ? 400 : 850}" height="${isMini ? 200 : 450}" viewBox="0 0 ${isMini ? 400 : 850} ${isMini ? 200 : 450}" xmlns="http://www.w3.org/2000/svg">
        <text x="${isMini ? 200 : 425}" y="${isMini ? 100 : 225}" font-family="-apple-system, BlinkMacSystemFont, sans-serif" font-size="20" fill="#f23645" text-anchor="middle">Error fetching data</text>
      </svg>
    `;
    res.setHeader('Content-Type', 'image/svg+xml');
