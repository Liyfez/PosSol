...
function generateSVG(symbol, values, metaData, periodLabel, themeName, isMini) {
  ...
  const themes = {
    ...
    venom: { bull: '#22c55e', bear: '#f3f4f6', grid: '#111827', text: '#ffffff', textMuted: '#9ca3af' },
    forest: { bull: '#a3e635', bear: '#d97706', grid: '#14532d', text: '#f0fdf4', textMuted: '#86efac' },
    volt: { bull: '#eab308', bear: '#f3f4f6', grid: '#18181b', text: '#ffffff', textMuted: '#a1a1aa' },
    ...
    ember: { bull: '#fb923c', bear: '#9f1239', grid: '#1a0500', text: '#fff1f2', textMuted: '#fda4af' },
    solar: { bull: '#facc15', bear: '#ec4899', grid: '#2e1065', text: '#fdf4ff', textMuted: '#e879f9' },
    monsoon: { bull: '#fb7185', bear: '#2dd4bf', grid: '#1c1917', text: '#fafaf9', textMuted: '#a8a29e' },
  ...
  const headerHtml = `
    ...
    <!-- Headers -->
    <g class="shadow">
      <text x="${pad.left}" y="${isMini ? 18 : 24}" font-size="${isMini ? 14 : 18}" font-weight="500" letter-spacing="-0.5"><tspan fill="#e2e8f0">${nameLabel}</tspan><tspan fill="${colors.text}"> · ${exchangeLabel}</tspan></text>
...
export default async function handler(req, res) {
  ...
  // Adding v17 to cache key to instantly bust the cache
  const cacheKey = `${symbol.toUpperCase()}_${numDays}d_${theme.toLowerCase()}_mini${isMini}_fake${isFake}_v17`;
